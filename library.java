...
public class library {
   ...
   public static void main( String [] args) throws Exception {
		...
		db = new databaseoperation("com.mysql.jdbc.Driver", "jdbc:mysql://localhost:3306/Project", "AS", "mypass");
	    // new object
		db.initDB();
	    //dropDatabase();
		//initDatabase();
		//loadDatabaseTable();
		 swingControlDemo = new MyGUI(); 
	...
	private static void dropDatabase() throws IOException, SQLException {
		   db.createDB("E:/Software/eclipse/workspace/JavaDB/src/database/tabledrop.sql");
	}
	private static void loadDatabaseTable() throws IOException, SQLException {
		   db.createDB("E:/Software/eclipse/workspace/JavaDB/src/database/tableinsertdata.sql");
	}
	private static void initDatabase() throws IOException, SQLException {
	       db.createDB("E:/Software/eclipse/workspace/JavaDB/src/database/tablebuild.sql");
	}
   ...
   public void showAdminMenu(){
       ...
       public void actionPerformed(ActionEvent e) {     
			...
			if (! adddocument.getText().isEmpty()) { 
                     ...
                     try {
                        ...
                        query = "INSERT INTO DOCUMENT (DOCID, TITLE, PDATE, PUBLISHERID) VALUES ("+ key + " , 'ENGLISH','2016-01-11 ','P1010')";
						db.ddlDB(query);
			 ...
			 // to be written
                 if (! searchdocument.getText().isEmpty()) { 
                         try {
                        	...
                        	query = "SELECT R.DOCID, R.RESNUMBER FROM RESERVES R WHERE R.DOCID = "
                        			+ docid + " UNION SELECT B.DOCID, B.BORNUMBER FROM BORROWS B WHERE B.DOCID =" + docid; 
                   ...
                   if (! addread.getText().isEmpty()) { 
                             try {
                            	 String key =  "'"+ adddocument.getText() + "RID9999'" ;
                             	 System.out.println(query);
                                 query = "INSERT INTO READER (READERID, RTYPE,RNAME, ADDRESS) VALUES (" + key + " , 'STAFF', 'ALEX', 'MUMBAI')";
    							 db.ddlDB(query);
      ...
      PrintBranchInfo.addActionListener(new ActionListener() {
       ...
       public void actionPerformed(ActionEvent e) {     
    	   try {
			 rs = db.queryDB("SELECT LNAME, LLOCATION FROM BRANCH");
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
     ...
     Top10borrowersBooks.addActionListener(new ActionListener() {
         ...
         public void actionPerformed(ActionEvent e) {     
      	   try {
  			 rs = db.queryDB("select count(b.docid) as doccount, b.readerid  from borrows b, book book " +
     ...
     Top10borrowedBooks.addActionListener(new ActionListener() {
         ...
         public void actionPerformed(ActionEvent e) {     
        	   try {
        			 rs = db.queryDB("select  b.docid, count(b.readerid) as doccount " +
     ...
     Top10PopularBooks.addActionListener(new ActionListener() {
    	 ResultSet rs;
          public void actionPerformed(ActionEvent e) {     
        	  try {
     			 rs = db.queryDB("select  b.docid, count(b.readerid)  from borrows b," +
   ...
   public void showReaderMenu(){
	      ...
	      insertcommands.addActionListener(new ActionListener() {
	       public void actionPerformed(ActionEvent e) {     
	            String query="";
	            /*
	             * for Query 1: Search document by ID / title / publisher name
	             * sample O/p
	             * 
	             */
				if (! searchdoc.getText().isEmpty()) { 
					 try {
                     	...
                     	query = "SELECT DOC.DOCID, DOC.TITLE,DOC.PDATE, PUB.PUBNAME FROM PUBLISHER AS PUB, DOCUMENT AS DOC WHERE PUB.PUBLISHERID = DOC.PUBLISHERID AND PUB.PUBNAME=" +docid + 
                                "UNION SELECT DOC.DOCID, DOC.TITLE,DOC.PDATE, PUB.PUBNAME FROM DOCUMENT AS DOC, PUBLISHER AS PUB WHERE PUB.PUBLISHERID = DOC.PUBLISHERID AND ( DOCID =" +docid +"  or " + "title =" +docid +")"; 
                     	ResultSet rs = db.queryDB(query);
                     	String data = "";
                  	   try {
              			while (rs.next()) {
	                 ...
	                 if (! doccheckout.getText().isEmpty()) { 
	                	 /*
	                	  * Query 2 . Insert based on  readerID , we assume readerid is constant READERID = RED51
	                	  * Insert DOCID : D2020
	                	  */
	                	 try {
	                   ...
	                   if (! docreserve.getText().isEmpty()) { 
	                	    ...
	                	    */
	                             try {
	                                 ...
	                                 query = "INSERT INTO RESERVES (READERID, DOCID, COPYNO, LIBID, DTIME) VALUES ('RED51', '" + key + "', 1, 'L1011', '2016-11-23 11:45:10')";
	    							 System.out.println(query);
	                                 db.ddlDB(query);
	                   ...
	                   if (! docreturn.getText().isEmpty()) {
	                	   /*
	                	    * Document return status , returns for document d2020
	                	    * shows  docid and return time
	                	    */
	                	   try {
	       ...
	       /*
	        * This button is to display the status 
	        * ==> what it returns ?     
	        */
	      ...
	      PrintDocreserve.addActionListener(new ActionListener() {
	       ...
	       public void actionPerformed(ActionEvent e) {     
	    	   try {
				 rs = db.queryDB("SELECT R.READERID, R.RESNUMBER AS RESNUMBER_BORNUMBER, R.DOCID, R.DTIME, B.BDTIME FROM RESERVES R  , borrows B WHERE R.READERID = 'RED51' OR B.READERID= 'RED51'");
			} catch (SQLException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
	     ...
	     publishlist.addActionListener(new ActionListener() {
	         ...
	         public void actionPerformed(ActionEvent e) {     
	      	   try {
	  			 rs = db.queryDB("SELECT D.DOCID, D.TITLE FROM DOCUMENT D, PUBLISHER P WHERE D.PUBLISHERID=P.PUBLISHERID AND P.PUBNAME='PEARSON';");
	  		} catch (SQLException e1) {
        e1.printStackTrace();
	  		}
	      	   String data = "";
	     ...
	     calcfine.addActionListener(new ActionListener() {
	         ...
	         public void actionPerformed(ActionEvent e) {     
	      	   try {
	  			 rs = db.queryDB("select readerid, case " +
